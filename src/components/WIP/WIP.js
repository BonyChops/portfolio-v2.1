        ...
        <div className="text-white bg-gray-900 pt-16 min-h-full font-bold min-h-screen h-full">
            <div className="py-16 xl:px-72 md:px-36 sm:px-16 px-4 xl:mx-36 md:mx-16 text-left bg-gray-900">
                <div className="flex py-4 border-b-2 border-gray-700 mb-6">
                    <div className="border-b-2 w-min mr-4 border-purple-400 text-purple-400">
                        WIP
                    </div>
                ...
                {wipData.map((v, i) => <button className={`w-full rounded-xl bg-gray-700 py-4 px-5 mb-2 text-left font-bold ${v.subtasks?.length <= 0 ? `cursor-default` : ""}`} onClick={e => openMenuList.includes(v.id) ? deleteMenuList(v) : addMenuList(v)}>
                    <div className="flex">
                        <div className="my-auto mr-4">
                            {v.loading ? <div className="w-8 h-8 rounded-xl bg-gray-800 animate-spin" /> : (v.subtasks.length > 0 ? (openMenuList.includes(v.id) ? MenuOpened : MenuClosed) : (v.recurring ? Refresh : (v.private ? Locked : Clipboard)))}
                        </div>
                        {v.loading ?
                            ...
                            <div className={`relative w-full text-left `}>
                                ...
                                <p className="text-xl font-normal">{(v.description !== "" && !v.private) && `${v.description}・`}Due {moment(v.due).locale("ja").format("MMM DD, YYYY hh:mm A")}</p>
                                <p className={`relative md:absolute md:right-0 md:bottom-0 w-auto ml-auto md:mr-0 ${moment(v.due) < moment() ? "text-red-600" : "text-purple-400"}`}>{moment(v.due) < moment() && "Past due"}{moment(v.due).format("YYYYMMDD") === moment().format("YYYYMMDD") && "Due today"}{moment(v.due).format("YYYYMMDD") === moment().add(1, "days").format("YYYYMMDD") && "Due tomorrow"}</p>
                                {v.subtasks?.length > 0 && (<div className="flex">
